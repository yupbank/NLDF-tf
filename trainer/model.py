...
def model(inputs, labels, feature_dim):
    with slim.arg_scope(vgg.vgg_arg_scope()):
        ...
        global_score = slim.conv2d(global_feature, num_outputs=2, kernel_size=[1, 1], padding='VALID')
        ...
        local_score = slim.conv2d(local_feature, num_outputs=2, kernel_size=[1, 1], padding='VALID', scope='ls')

        prob = tf.nn.softmax(local_score + global_score)
